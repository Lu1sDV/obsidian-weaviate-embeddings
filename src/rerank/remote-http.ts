...
function statusError(status: number): RerankError {
  if (status === 401 || status === 403) return new RerankError("authentication", false, status);
  if (status === 400 || status === 422) return new RerankError("contract", false, status);
  if (status === 429) return new RerankError("rate-limit", true, status);
  return new RerankError("provider", status >= 500, status);
}

/** Fixed-host cloud transport. It never follows redirects and never includes response bodies in errors. */
  ...
  return ({ provider, apiKey, body, signal, deadlineAt, beforeSend }) => new Promise((resolve, reject) => {
    ...
    try {
      ...
      req = makeRequest(new URL(PROVIDERS[provider].url), {
        ...
        if (status !== 200) {
          finish(statusError(status));
          return;
        }
        const encoding = incoming.headers["content-encoding"];
        const length = incoming.headers["content-length"];
        if ((encoding && encoding !== "identity")
          || (length !== undefined && (!/^\d+$/.test(length) || Number(length) > LIMITS.responseBytes))) {
          finish(new RerankError("invalid-response"));
          return;
        }
        ...
        incoming.on("end", () => {
          if (!incoming.complete) {
            finish(new RerankError("provider", true));
            return;
          }
          try {
      ...
      if (signal.aborted) { abort(); return; }
      // Authorization/currentness is checked again immediately before the write.
      beforeSend();
      req.end(body);
    } catch (error) {
      finish(error instanceof RerankError ? error : new RerankError("provider"));
    }
  });
}
